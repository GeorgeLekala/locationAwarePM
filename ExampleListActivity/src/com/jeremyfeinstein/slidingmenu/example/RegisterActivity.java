...
public class RegisterActivity extends Activity implements  android.widget.AdapterView.OnItemSelectedListener{
	...
	*/
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        final EditText fname = (EditText) findViewById(R.id.reg_txtfirstname);
        final EditText lname = (EditText) findViewById(R.id.reg_txtlastname);
        final EditText username = (EditText) findViewById(R.id.reg_txtusername);
        final EditText password = (EditText) findViewById(R.id.reg_txtpassword);
        final EditText cpassword = (EditText) findViewById(R.id.reg_txtcpassword);
	    ...
	    buttonRegister.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
        		...
        		try {
        			String tmpPass = cpassword.getText().toString();
        			if(!password.getText().toString().equals(tmpPass))
        			{
        			...
        			{
        				mGetUserItem.setmId("");
        				mGetUserItem.setMfirstname(fname.getText().toString());
        				mGetUserItem.setMlastname(lname.getText().toString());
        				mGetUserItem.setmUsername(username.getText().toString());
        				mGetUserItem.setmPassword(SHA1(password.getText().toString()));
        				mGetUserItem.setmCommand("IGNORE");
        				mGetUserItem.setmWorkgoup(client_wg);
        				mGetUserItem.setmWorkgoup_state("IGNORE");
        				mGetUserItem.setMauth_level("1");
        				mGetUserItem.setComputername("IGNORE");
        				mGetUserItem.setTerminateprocess("IGNORE");	
		...
		mCommandTable.insert(item, new TableOperationCallback<CommandItem>() {
			...
			public void onCompleted(CommandItem entity, Exception exception, ServiceFilterResponse response) {
				...
				if (exception == null) {
					...
					Toast.makeText(getApplicationContext(),"Try Agin : Failed",Toast.LENGTH_LONG).show();
		...
		mCommandTable.where().field("username").eq(val(uname)).execute(new TableQueryCallback<CommandItem>() {
			...
			public void onCompleted(List<CommandItem> result, int count, Exception exception, ServiceFilterResponse response) {
				if (exception == null) {
					...
					Toast.makeText(getApplicationContext(),"Authentication: "+exception.getMessage(),Toast.LENGTH_LONG).show();
		...
		mCommandTable.where().field("username").eq(val(uname)).execute(new TableQueryCallback<CommandItem>() {
			...
			public void onCompleted(List<CommandItem> result, int count, Exception exception, ServiceFilterResponse response) {
				if (exception == null) {
					...
					Toast.makeText(getApplicationContext(),"getUser: "+exception.getMessage(),Toast.LENGTH_LONG).show();
